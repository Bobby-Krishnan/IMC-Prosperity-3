...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # --------- KELP STRATEGY (from KTest.py +389) ---------
        kelp_price_history = []
        ...
        kelp_volatility = (
            np.std(kelp_price_history)
            if len(kelp_price_history) >= 2
            else 0
        )
        ...
        # Buy condition
        if (
            kelp_best_ask is not None
            and kelp_best_ask < kelp_fair_price - kelp_volatility * 0.5
            and kelp_position < self.kelp_position_limit
        ):
            kelp_ask_volume = -kelp_depth.sell_orders[kelp_best_ask]
            kelp_buy_volume = min(
                kelp_ask_volume, self.kelp_position_limit - kelp_position, kelp_trade_volume
            )
        ...
        # Sell condition
        if (
            kelp_best_bid is not None
            and kelp_best_bid > kelp_fair_price + kelp_volatility * 0.5
            and kelp_position > -self.kelp_position_limit
        ):
            kelp_bid_volume = kelp_depth.buy_orders[kelp_best_bid]
            kelp_sell_volume = min(
                kelp_bid_volume, kelp_trade_volume, self.kelp_position_limit + kelp_position
            )
