...
class Trader:
    def run(self, state: TradingState):
        ...
        traderDataOut.setdefault("pnl", 0)
        traderDataOut.setdefault("entry_book", {})

        orders_to_place = []
        rock_price = None

        # === Volcanic Rock Mean Reversion ===
        ...
        # === Vouchers Pricing and Arbitrage ===
        ...
        if len(data) >= 20:
            ...
            for product, strike in STRIKE_MAP.items():
                ...
                running_pnl = traderDataOut["pnl"]
                max_qty = 2 if running_pnl < -1000 else 3 if running_pnl < 0 else MAX_TRADE_QTY
                qty = min(max_qty, max(1, int(confidence * max_qty)))

                entry_book = traderDataOut["entry_book"].get(product, {})
                if product in traderDataOut["entry_book"]:
                    age = state.timestamp - entry_book.get("timestamp", state.timestamp)
                    unrealized = (market_price - entry_book["price"]) * position
                    if age > MAX_HOLD_TICKS or unrealized < -MAX_UNREALIZED_LOSS:
                        close_qty = -position
                        orders_to_place.append(Order(product, best_bid if position > 0 else best_ask, close_qty))
                        traderDataOut["entry_book"].pop(product, None)
                        continue

                if confidence < MIN_CONFIDENCE:
                    continue

                if market_price < 0.92 * theo_price and position + qty <= position_limit:
                ...
                elif market_price > 1.08 * theo_price and position - qty >= -position_limit:
